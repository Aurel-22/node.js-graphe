/**
 * Crée un graphe de 5 000 nœuds avec une structure multi-communautés réaliste.
 * 
 * Structure :
 * - 10 communautés de ~500 nœuds chacune
 * - Connexions denses INTRA-communauté (chaque nœud a 3-8 voisins locaux)
 * - Ponts INTER-communautés (hubs qui relient les clusters)
 * - ~25 000 arêtes au total (~5 par nœud en moyenne)
 * 
 * Utilise UNWIND pour des batch inserts rapides (2 requêtes au lieu de 5000+)
 */
...
const GRAPH_ID = 'community_5k';
const NODE_COUNT = 5000;
const COMMUNITY_COUNT = 10;
const NODES_PER_COMMUNITY = NODE_COUNT / COMMUNITY_COUNT; // 500
